...
if uploaded_file is not None:     
    ...
    df = data.loc[data.groupby(['WAYS']).Duration_mins.idxmin()]
    Best_Route = df[["Origin","Destination ",'WAYS','DISTANCE(METERS)','Duration_mins']]    
    st.info("Best Route with less traffic")
    if st.button("click here for Best Route"):
     st.write(df[["Origin","Destination ",'WAYS','DISTANCE(METERS)','Duration_mins']] )

    st.subheader("metrics of louvain_communities")
